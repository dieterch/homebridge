...
function init( params ) {
    ...
    setTimeout( () => {
        let msg = `${config.name} handled in hue.js. ${config._ ? config._ : ''}`;
        log( msg );
        config.url = config.url ? config.url : "http://localhost:1883"
        //log( JSON.stringify(config))
        //let state = 'tadaaaa'
        // update state
        //notify( 'on', config.onValue || "ON" );
        //notify( 'brightness', config.brightness );
        //notify( 'HSV', '0,50,50' );
    }, 1000 );
    ...
    function encode_on( message, info, output) {
        log( `encode_on called for topic [${info.topic}], property [${info.property}] with message [${message}]` );
        output( message );
    }
    ...
    function encode_HSV( message ) {
        ...
        var b=2.54*params[2];
        result.brightness=Math.round(b);
